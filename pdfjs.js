(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.pdfjs) {
      window.wwload.pdfjs = "wait";
      /*requirejs.config({
        paths: {
          "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode   PDFJS,workerSrc,getDocument
        }
      });*/
      require(["libs/pdfjs-dist/build/pdf"], function(PDFJS) {
        window.PDFJS = PDFJS;
        window.wwload.pdfjs = true;       
        replace();
        fncallback();
      });
    } else if (window.wwload.pdfjs === "wait") {
      setTimeout(function () {
   ...
   */
   function pdfjs1($ele){
    var config = {};
    config.link = $ele.attr("data--link");             //设置每个pdf链接 
    //$ele.find('.showPdf').click(function(){
      var pdf_url = '/libs/pdfjs-dist/web/viewer.html?file='+ config.link;
      $ele.find('.showPdf').attr("src",pdf_url);                 //动态添加地pdf超链接
      console.log(pdf_url);
   //});
